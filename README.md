...
Первый стенд развёрнут и проверен 22.09.2026; адрес текущего стенда выводит
`terraform output site_url` (после пересоздания IP меняется).
...
- [Подготовка нового компьютера](#подготовка-нового-компьютера)
...
Git содержит исходники, шаблоны и примеры параметров. Клонирование не переносит:

- Terraform, YC CLI и Python venv;
- авторизацию YC и IAM-токен;
- реальные параметры облака `terraform/terraform.tfvars.json`;
- SSH-ключи `.local/otus_dz03` и `.local/otus_dz03.pub`;
- пароли и соли приложения `.local/secrets.yml`;
- локальный `terraform/terraform.tfstate`.

Сначала выбрать сценарий ниже, затем выполнять инструкцию своей ОС.
State хранится локально; remote backend и CI/CD для этого ДЗ не требуются.
Ansible работает в venv на балансировщике, поэтому на Windows WSL не нужен.
...
### Продолжение работы с существующим стендом на другом компьютере

Один git clone не даёт возможности управлять существующими ВМ. До plan/apply:

1. Остановить операции Terraform на прежнем компьютере и сделать резервную копию
   актуального state. Передать по защищённому каналу `terraform/terraform.tfstate`,
   его резервную копию, `terraform/terraform.tfvars.json`, `.local/otus_dz03`,
   `.local/otus_dz03.pub` и `.local/secrets.yml` в соответствующие каталоги нового проекта.
2. Установить инструменты и создать venv для новой ОС. Не переносить `.tools`,
   `.venv`, `.terraform` и сохранённые планы; `.terraform.lock.hcl` берётся из Git.
3. Авторизоваться в том же cloud/folder и получить новый IAM-токен.
   Существующие SSH-ключи, пароли и соли не заменять вновь созданными.
4. Проверить пути в tfvars и `admin_cidrs` для нового рабочего места.
   Выполнить `init`, `state list` и `plan`. План не должен неожиданно предлагать
   создание второго комплекта ВМ или замену существующих. Изменение адреса
   администратора может потребовать обновления группы безопасности.
5. После переноса использовать один компьютер как владельца рабочего state.
   Предыдущую копию оставить резервной, не запускать Terraform одновременно
   с двух компьютеров. Передать управление обратно можно только с актуальным state.

Эти действия одинаково применимы к Windows и Linux. Ограничить доступ к state
и секретам текущим пользователем: права файлов в Linux (`chmod 600`) или ACL
в Windows. Проверенные host keys можно заново получить командой `controller.py trust`;
`prepare` и `deploy` также получают их автоматически через YC.
...
Если продолжается существующий стенд, сначала перенести актуальные
state, tfvars, ключи и секреты по [общей инструкции переноса](#продолжение-работы-с-существующим-стендом-на-другом-компьютере).
