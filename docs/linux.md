...
Порядок переноса state, параметров, ключей и секретов описан в
[общем разделе README](../README.md#продолжение-работы-с-существующим-стендом-на-другом-компьютере).
Он применим к Windows и Linux. Для нового независимого стенда старый state не нужен.
